...
Parser.prototype = {
    ...
    block: function(){
        ...
        this.skip('WS');
        while(this.la() !== '}'){
            var declareOrStmt = this.mark().declaration() || this.restore().stmt(); 
        ...
        while(true){
            ...
            if(ll.type === '#{' && this.ll(2) !== '}'){
                ...
                if(ll.type === 'DIMENSION'){
                    if(ll.unit !== '%') this.error('invalid selector element: ' + ll.type +':'+ll.value, ll.lineno)
                }
    ...
    func: function(){
        var param, params = [], 
            rest = 0, block, lineno = this.ll().lineno;
        if(this.eat('(')){
            this.eat('WS');
            if(this.la() !== ')'){
                do{
